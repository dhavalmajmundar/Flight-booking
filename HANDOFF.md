...
- Root cause of the FlightCompanion app's "Request failed" errors turned out
  to be broader than the Telegram outage below: `docker-compose.oracle.yml`
  had never exposed `flight-booking` publicly at all — no port mapping, no
  Traefik labels, no domain. Both client app installs had been pointed at
  the Railway URL the whole time; the app's earlier 502 was Railway serving
  a crashing/conflicted deployment, and once that Railway service was
  removed (see root-cause entry below) the same URL correctly started
  returning 404 instead ("no deployment here"). Oracle was never actually
  serving either app.
- Confirmed via `docker inspect` on the working `shivora-website` Coolify
  container (`dog3ed8jhwgiwthur3rmk2zx-152314252560`) that Coolify's
  Traefik proxy only auto-configures domains/TLS for *Managed* resources.
  `flight-booking` is Unmanaged (a hand-run `docker compose` project), so
  routing has to be hand-written into `docker-compose.oracle.yml` rather
  than generated by Coolify's UI.
- Added a DNS A record on Namecheap: `flightbot.shivoralabs.com` →
  `150.136.102.196` (the Oracle VM).
- Added `traefik.*` labels to the `flight-booking` service mirroring the
  working `shivora-website` pattern exactly (same `entryPoints` names
  `http`/`https` and `tls.certresolver=letsencrypt`, both confirmed from
  that container's live labels rather than assumed): HTTP redirects to
  HTTPS, HTTPS terminates TLS via Let's Encrypt and forwards to this
  container's port 8080 (`flight_bot/main.py`, `PORT` in `.env.oracle`).
  Attached the service to the external `coolify` Docker network so
